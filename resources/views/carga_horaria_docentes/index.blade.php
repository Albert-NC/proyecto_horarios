...
>
        ...
        <div>
            ...
            <p class="text-sm"
               :class="tema === 'oscuro' ? 'text-gray-400' : 'text-slate-600'">
                Aquí se registran las horas, categoría y modalidad de cada docente.
            </p>
        </div>

        <a href="{{ route('carga-horaria-docentes.create') }}"
    ...
    <div class="flex items-center gap-2 mb-4 text-sm">
        ...
        <a href="{{ route('carga-horaria-docentes.index') }}"
           ...
           :class="tema === 'oscuro'
                ? '{{ $lugarActual ? 'border-white/15 text-gray-300 hover:bg-white/5'
                                   : 'bg-fuchsia-600 text-white border-fuchsia-500' }}'
                : '{{ $lugarActual ? 'border-violet-200 text-slate-700 hover:bg-violet-50'
                                   : 'bg-fuchsia-600 text-white border-fuchsia-500' }}'">
            Todos
        </a>

        <a href="{{ route('carga-horaria-docentes.index', ['lugar' => 'Trujillo']) }}"
           class="px-3 py-1 rounded-full border text-xs font-medium transition"
           :class="tema === 'oscuro'
        ...
        <a href="{{ route('carga-horaria-docentes.index', ['lugar' => 'Valle']) }}"
           class="px-3 py-1 rounded-full border text-xs font-medium transition"
           :class="tema === 'oscuro'
                ? '{{ $lugarActual === 'Valle'
                        ? 'bg-fuchsia-600 text-white border-fuchsia-500'
        ...
        <table class="min-w-full text-sm text-left">
            <thead
                ...
                <tr>
                    ...
                    <th class="py-2.5 px-4">Horas</th>
                    <th class="py-2.5 px-4">Modalidad</th>
                    <th class="py-2.5 px-4 text-right">Acciones</th>
                </tr>
            </thead>
            <tbody :class="tema === 'oscuro' ? 'text-gray-100' : 'text-slate-900'">
                @forelse ($docentes as $docente)
                    <tr class="border-b last:border-0 transition"
                        ...
                        <td class="py-2.5 px-4">
                            {{ $docente->categoria ? ucfirst($docente->categoria) : '-' }}
                        </td>
                        <td class="py-2.5 px-4">
                            {{ $docente->horas ?? 0 }}
                        </td>
                        <td class="py-2.5 px-4">
                            {{ $docente->modalidad ? ucwords($docente->modalidad) : '-' }}
                        </td>
                        <td class="py-2.5 px-4 text-right">
                            <a href="{{ route('carga-horaria-docentes.edit', $docente) }}"
                ...
                @empty
                    <tr>
                        <td colspan="6" class="py-4 px-4 text-center"
                            ...
                            No hay carga horaria registrada todavía.
                        </td>
                    </tr>
                @endforelse
            </tbody>
...
{{-- BOTÓN FLOTANTE IMPORTAR EXCEL --}}
<button
    type="button"
    @click="$refs.modalImportarCarga.showModal()"
...
>
    ...
    <form
        method="POST"
        action="{{ route('carga-horaria-docentes.importar') }}"
    ...
    >
        ...
        <p class="text-xs text-gray-300">
            ...
            <strong>Código</strong>, <strong>Categoria</strong>,
            <strong>Horas</strong>, <strong>Modalidad</strong>.
        </p>
        ...
        <div class="flex justify-end gap-2 pt-2 border-t border-white/10">
            <button
                ...
                @click="$refs.modalImportarCarga.close()"
            >
                Cancelar
            </button>
